...
{
    // 为实时数据创建共享内存
    m_rtdbSharememory.setName("runtime");
    if(m_rtdbSharememory.create(sizeof(TDBTagObject) * MAX_DBTAGOBJECT)) {
        LogInfo("create sharememory[runtime] success!");
        m_pDBTagObjectBaseAddr = (PDBTagObject)m_rtdbSharememory.open();
        if(NULL == m_pDBTagObjectBaseAddr) {
            LogError("get sharememory[runtime] pointer fail!");
            m_bMemStatus = false;
        } else {
            LogInfo("get sharememory[runtime] pointer success!");
            for(int i=0; i<MAX_DBTAGOBJECT; i++) {
                memset(&m_pDBTagObjectBaseAddr[i], 0, sizeof(TDBTagObject));
            }
            LogInfo("clear sharememory[runtime] success!");
        }
    } else {
        LogError("create sharememory[runtime] fail!");
        m_bMemStatus = false;
    }
}
...
PDBTagObject RealTimeDB::getEmptyDBTagObject()
{
    for(int i=0; i<MAX_DBTAGOBJECT; i++) {
        ...
        int SizeofTDBTagObject = sizeof(TDBTagObject);
        for(int j=0; j < SizeofTDBTagObject; j++) {
            iSum += pData[j];
        }
        if(iSum == 0) {
...
//    }
    ...
    for (int i = 0; i < DbData::NodeInfo_Count; i++) {
        ...
        float nodeValue = RealTimeDB::GetDataString(positionID).toFloat();
        //例如某温度传感器和变送器检测的是-10－60℃，对应数值是6400－32000,用上述的方程表达为X=70*(Z-6400)/25600-10
        ...
        if (DbData::NodeInfo_NodeValue.at(i) != nodeValue) {
            DeviceHelper::deviceValue(DbData::NodeInfo_PositionID.at(i), nodeValue);
            DbData::NodeInfo_NodeValue[i] = nodeValue;
            //更新在线状态
            DeviceHelper::deviceOnline(DbData::NodeInfo_DeviceName.at(i), true);
            DbData::NodeInfo_Online[i] = true;
        }
    }
}
