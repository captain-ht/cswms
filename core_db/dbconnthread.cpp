...
bool DbConnThread::openDb()
{
    ...
    if (dbOk) {
        ...
        emit error(QString("打开%1数据库失败").arg(dbFlag));
...
void DbConnThread::execSqlPlus()
{
    ...
    if (database.commit()) {
        ...
        database.rollback();
        dbOk = false;
        emit error(QString("%1数据库实时更新数据提交事务失败%2").arg(dbFlag).arg(msg));
        qDebug() << TIMEMS << this->objectName() << database.lastError();
    }
}
...
void DbConnThread::execSqlUpdate()
{
    ...
    if (database.commit()) {
        ...
        database.rollback();
        dbOk = false;
        emit error(QString("%1数据库批量更新数据提交事务失败%2").arg(dbFlag).arg(msg));
        qDebug() << TIMEMS << this->objectName() << database.lastError();
    }
}
...
void DbConnThread::initDb(const QStringList &sqls)
{
    ...
    if (dbOk && count > 1) {
        ...
        if (database.commit()) {
            ...
            database.rollback();
            dbOk = false;
            emit error(QString("%1数据库初始化数据提交事务失败%2").arg(dbFlag).arg(msg));
            qDebug() << TIMEMS << this->objectName() << database.lastError();
        }
    }
}
