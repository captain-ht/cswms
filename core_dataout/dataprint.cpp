...
void DataPrint::appendHead()
{
    ...
    list.clear();
    //表格开始
    list << "<table border='0.5' cellspacing='0' cellpadding='3'>";
}
...
void DataPrint::appendColumn()
{
    ...
    if (columnCount > 0) {
        ...
        if (dataContent.stretchLast) {
            for (int i = 0; i < columnCount - 1; i++) {
                list << QString("<td width='%1' align='center'>%2</td>").arg(dataContent.columnWidths.at(i)).arg(dataContent.columnNames.at(i));
            }
            //最后一列自适应宽度
            list << QString("<td width='%1' align='center'>%2</td>").arg("100%").arg(dataContent.columnNames.at(columnCount - 1));
        } else {
    ...
    for (int i = 0; i < rowCount; i++) {
        ...
        if (exist) {
            for (int j = 0; j < columnCount; j++) {
                list << QString("<td width='%1' align='center' style='vertical-align:middle;'><font color='%3'>%2</font></td>")
                     .arg(dataContent.columnWidths.at(j)).arg(value.at(j)).arg(dataContent.checkColor);
            }
        } else {
            for (int j = 0; j < columnCount; j++) {
                list << QString("<td width='%1' align='center' style='vertical-align:middle;'>%2</td>")
                     .arg(dataContent.columnWidths.at(j)).arg(value.at(j));
            }
        }
