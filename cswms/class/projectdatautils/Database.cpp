...
Database::Database(const QString &dbnamePrj,
...
int Database::getRowCount(QSqlQuery query, const QString &tableName, const QString &expr) {
    int iCount = 0;
    QString szSql = "";
    QSqlRecord rec;
...
{
    return dbPrj_.driver()->beginTransaction();
}
...
{
    return dbPrj_.driver()->rollbackTransaction();
}
...
{
    return dbPrj_.driver()->commitTransaction();
}
...
{
    static QStringList szListTable = dbPrj_.tables();
