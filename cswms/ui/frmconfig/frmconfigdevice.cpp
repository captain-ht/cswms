...
void frmConfigDevice::initData()
{
    ...
    //初始化列名和列宽
    columnNames << "编号" << "端口名称" << "控制器名称" << "控制器地址" << "控制器型号" << "变量数量";
...
void frmConfigDevice::numberValueChanged(const QString &value)
{
    ...
    if (index >= 0) {
        ...
        if (value.toInt() > nodeNumber) {
            model->setData(model->index(row, 5), nodeNumber);
            QUIHelper::showMessageBoxError(QString("当前控制器最大变量数量为 %1 !\n请重新输入!").arg(nodeNumber));
        }
    }
}
...
void frmConfigDevice::dataout(int type)
{
...
void frmConfigDevice::on_btnXls_clicked()
{
    dataout(2);
}
