...
* 从文件读取配置数据
*/
bool ComDevicePrivate::LoadData(int id)
{
    m_sDeviceName = DbData::PortInfo_PortName.at(id);
    m_iFrameLen = 64;
    m_sProtocol = DbData::PortInfo_Protocol.at(id);
    m_sLink = "主链路";
    m_iStateVar = 0;
    m_iFrameTimePeriod = 50;
    m_iCtrlVar = 0;
    m_bDynamicOptimization = 0;
    m_iRemotePort = 0;
    m_iRetryTimes = 3;
    m_iCommTimeout = 1000;
    m_iCommResumeTime = 15000;
    m_sDeviceType = "COM";
    m_sPortNumber = DbData::PortInfo_ComName.at(id);
    m_iBaudrate = DbData::PortInfo_BaudRate.at(id);
    m_iDatabit = 8;
    m_fStopbit = 1;
    m_sVerifybit = "无校验";
    m_iTimeout = DbData::PortInfo_ReadTimeout.at(id);
...
* 从文件读取配置数据
*/
bool NetDevicePrivate::LoadData(int id)
{
    m_sDeviceName = DbData::PortInfo_PortName.at(id);
    m_iFrameLen = 64;
    m_sProtocol = DbData::PortInfo_Protocol.at(id);
    ...
    m_iCommResumeTime = 15000;
    m_sDeviceType = "NET";
    m_sIpAddress = DbData::PortInfo_TcpIP.at(id);
    m_iPort = DbData::PortInfo_TcpPort.at(id);
    m_sIpAddress1 = "0";
    m_iPort1 = 0;
...
//-----------------------------------------------------------------------------
QScopedPointer<Vendor> Vendor::self;
Vendor *Vendor::Instance()
{
    if (self.isNull()) {
        ...
        if (self.isNull()) {
            self.reset(new Vendor);
        }
    }

    return self.data();
}
Vendor::Vendor(QObject *parent) : QObject(parent)
{
            ...
            if(pTag->getBlockReadTagId() == "block") { // 块读取变量
                if(m_pVendorPluginObj->readIOTag(this, m_pPortObj, pTag) != 1) {
                    ...
                    if(iRetryTimes >= m_pVendorPrivateObj->m_iRetryTimes) {
                        if(pTag->isReadBlockReadTagSuccess()){
                            emit receiveOnline(getDeviceName(), pTag->GetDeviceAddress(), false);
                        }
                ...
                if(!pTag->isReadBlockReadTagSuccess()){
                    emit receiveOnline(getDeviceName(), pTag->GetDeviceAddress(), true);
                }
